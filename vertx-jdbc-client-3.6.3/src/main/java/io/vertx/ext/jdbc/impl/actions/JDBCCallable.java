    ...
    try (CallableStatement statement = conn.prepareCall(sql)) {
      ...
      if (retResult) {
        ...
        if (outResult) {
          ...
          try {
            JsonObject OracleResultSetObject = resultSet.getOutput().getJsonObject(out.size() - 1);
            return new io.vertx.ext.sql.ResultSet(OracleResultSetObject).setOutput(resultSet.getOutput());
          } catch (Exception ex) {
            return resultSet;
          }
        }
