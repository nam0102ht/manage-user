...
public class WorkFlowDeleteProduct extends TaskWorkFlow {

    public WorkFlowDeleteProduct(String name, Vertx vertx) {
        super(name, vertx);
        addTask(new CheckUserRoleTask("CheckUserRoleTask", vertx));
        addTask(new AbstractTask("DeleteUserTask", vertx) {
            @Override
            protected void exec(TaskData input, Handler<TaskData> whenDone) {
                ...
                ProductsDBTask productDeleteTask =
                        new ProductsDBTask("ProductsDBTask", vertx, TypeCheck.DELETE);
                productDeleteTask.run(input, async -> {
